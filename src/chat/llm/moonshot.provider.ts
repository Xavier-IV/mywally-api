...
export class MoonshotProvider implements LlmProvider {
  ...
  private model = 'moonshot-v1-32k';

  constructor(private readonly config: ConfigService) {
    const apiKey = this.config.get<string>('MOONSHOT_API_KEY');
    this.model = this.config.get<string>('MOONSHOT_MODEL') ?? this.model;
    const baseURL = this.config.get<string>('MOONSHOT_BASE_URL') ?? 'https://api.moonshot.ai/v1';
    if (apiKey) {
      this.client = new OpenAI({ apiKey, baseURL });
      this.logger.log(`Moonshot client ready (model=${this.model}, baseURL=${baseURL})`);
    } else {
      this.logger.warn('MOONSHOT_API_KEY not set');
    }
  }
  ...
  async generate(input: LlmGenerateInput): Promise<LlmGenerateOutput> {
    if (!this.client) throw new Error('Moonshot client is not configured');
